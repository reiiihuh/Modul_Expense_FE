...
export default {
  ...
  methods: {
    async handleLogin() {
      try {
        // Kirimkan request ke API login
        const response = await axios.post('http://localhost:5000/api/login', {
          username: this.username,
          password: this.password,
        });

        // Jika login berhasil, simpan token dan arahkan ke halaman lain
        const token = response.data.accessToken;

        // Simpan token di localStorage (atau vuex jika lebih kompleks)
        localStorage.setItem('token', token);

        // Redirect ke halaman Home
        this.$router.push('/Home');
      } catch (error) {
        // Tangani error, misal username atau password salah
        alert('Username atau Password salah, atau masalah pada server.');
        console.error(error);
      }
    },
  },
